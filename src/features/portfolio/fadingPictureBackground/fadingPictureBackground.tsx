...
export default function FadingPictureBackground() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const maxScroll = window.innerHeight; // Adjust based on how quickly you want the fade
      const newOpacity = Math.max(1 - scrollY / maxScroll, 0); // Fade out as you scroll
  ...
  return (
    ...
    >
      <img
        className="fixed left-0 -z-0 h-screen w-full object-cover"
        src={natureImage1}
        alt="nature"
        style={{ opacity: opacity }}
      />
      <div className="relative top-0 z-10 flex flex-row justify-between md:text-xl sm:text-lg">
        <div className="lg:ml-12 md:ml-12 sm:ml-3 ml-2 flex h-screen flex-col flex-wrap justify-between gap-10 overflow-visible min-w-0">
          <h1 className="mt-20 text-wrap text-4xl">
            Suomalaista ohjelmistotekniikkaa
          </h1>
          <div className="text-card mb-40 flex flex-col gap-2">
